<?php
	...
	$id_blog_classificados = mysqli_real_escape_string($conn, $_POST['id']);
	$classificados_album = mysqli_real_escape_string($conn, $_POST['produto_album']);
	
	
	//cria a pasta onde serão colocados os arquivos
		//Pasta onde o arquivo vai se salvo
		$_UP['pasta'] = '../../../../imagem_produto/'.$classificados_album.'/';
		...
		if(array_search($extensao, $_UP['extensoes']) === false){	
			 echo "
				...
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../../../administrativo.php?link=57&id=".$id_blog_classificados."'>
		...
		//O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta arquivo
			...
			if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'].$nome_final)){
				...
				$result_produtos = "INSERT INTO fotos_produtos (id_produto, fotos_produto) VALUES ('$id_blog_classificados', '$nome_final')";
